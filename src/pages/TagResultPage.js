...
const TagResultPage = () => {
    ...
    const tagFromUri = queryParameters.get("tag");
    ...
    useEffect(() => {
        ...
        .then(data => {
            const filteredData = data.filter(item => {
                return item.tags.some(tag => tag.name.includes(tagFromUri));
            });
    ...
    return (
        <div className='dark:bg-slate-600 h-screen flex flex-col items-center justify-start'>
            <h1 className='mt-8 dark:text-white'>{language === 'en' ? en.searchFor : ru.searchFor}: #{tagFromUri}</h1>
